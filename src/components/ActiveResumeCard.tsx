...
export default function ActiveResumeCard({ resume }: ActiveResumeCardProps) {
  return (
    <View style={[AppStyles.jobCard, { marginTop: 12, backgroundColor: '#e6f0fa', padding: 12 }]}>
      <Text style={[AppStyles.jobTitle, { fontSize: 16, marginBottom: 6 }]}>✅ Активное резюме</Text>
      <Text style={{ fontWeight: 'bold', fontSize: 16 }}>{resume.title}</Text>
      <Text style={{ color: Colors.secondary, fontSize: 14, marginBottom: 6 }}>{resume.position}</Text>
      {resume.file_url && (
        <TouchableOpacity onPress={() => Linking.openURL(resume.file_url!)} style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
          ...
          <Text style={{ color: Colors.primary, fontSize: 12 }}>{resume.file_name ? resume.file_name.substring(0, 30) : 'PDF загружен'}</Text>
        </TouchableOpacity>
      )}
    </View>
  );
